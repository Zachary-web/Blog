...
<script setup>
// 1. 导入 Vue 3 核心 API 和 Vuex 4 的 useStore（若用 Pinia 需调整，见下方说明）
import { useStore } from 'vuex'
import { defineProps, defineOptions } from 'vue'

// 2. 定义组件名（Vue 3 <script setup> 默认无 name，需用 defineOptions 显式声明，可选但建议保留）
defineOptions({
  name: 'blogFooter'
})

// 3. 定义 Props（替代 Vue 2 的 props 选项，类型校验逻辑完全一致）
...
// 4. 获取 Vuex Store 实例（替代 Vue 2 的 this.$store）
const store = useStore()

// 5. 定义方法（直接声明函数即可，无需嵌套在 methods 选项中）
const toBlog = (blog) => {
  store.dispatch('goBlogPage', blog) // 保持原 dispatch 逻辑不变
}
</script>
